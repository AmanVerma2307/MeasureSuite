...
class sensorSimulator():
    ...
    def scoreMatrix(self):
        ...
        """
        Function to get avg. quantifier scores, disentanglement amt., and ground truth values
        """
        ...
        for sensorIdx in range(len(self.embeddingList)):
            for modelIdx in range(len(self.dataList[0])):
                ...
                y_dev, y_dev_id, G_total, I_total, groundVal_curr = self.getData(dataCurr)
            ...
            groundVal_curr = (groundVal_curr - np.mean(groundVal_curr))/np.std(groundVal_curr)
            if(normalize == 1):
                groundVal_curr = groundVal_curr/np.linalg.norm(groundVal_curr)

            groundVal_curr = np.mean(groundVal_curr)
            groundVal.append(groundVal_curr)

        return scores, disentScores, np.array(groundVal)


if __name__ == "__main__":

    senSim = sensorSimulator('bdb',quantifier='deltaDistance')
    scores, disentScores, groundVal = senSim.scoreMatrix()
    print(scores, disentScores, groundVal)
    print(scores.shape,
          disentScores.shape,
          groundVal.shape)